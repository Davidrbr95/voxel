...
voxel_path = r"C:\Users\ARPA\Desktop\ControlCodes\UHR-OTLS-control-Dec2024\voxel"
sys.path.append(voxel_path)

import unittest
from voxel.devices.camera.sdks.dcam.dcam import Dcamapi, Dcam
from voxel.devices.camera.hamamatsu_pylablib import Camera  



# Constants for the tests 
SERIAL_NUMBER = 306727
WIDTH_PX = 2048
HEIGHT_PX = 2048
...
PIXEL_TYPES = {'mono16': 2, 'mono12': 3} #'mono8': 1, 
# TRIGGERS = {
...
class TestCamera(unittest.TestCase):
    ...
    def test_camera_initialization(self):
        ...
        self.assertIsNotNone(self.camera._cam)
    ...
    def test_camera_set_exposure_time(self):
        ...
        self.camera.exposure_time_ms = 10
        exposure_time_set = self.camera.exposure_time_ms
        print(f"Exposure time set and verified: {exposure_time_set} ms")
        self.assertAlmostEqual(exposure_time_set, 10, DECIMAL_PLACE)
    ...
    def test_camera_set_pixel_type(self):
        ...
        for pixel_type, value in PIXEL_TYPES.items():
            with self.subTest(pixel_type=pixel_type):
                print('STarting type', self.camera.pixel_type)
    ...
    def test_camera_set_height_px_offset(self):
        ...
        while frame is None:
            frame = self.camera.getFrame()
        print('heigh px should be 400', frame.shape)
        self.camera.stop()
        self.assertEqual(self.camera.height_px, 400)
